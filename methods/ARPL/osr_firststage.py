...
from methods.ARPL.core import train, train_cs, test , trainmixup
...
parser.add_argument('--checkpt_freq', type=int, default=200)
...
parser.add_argument('--projdim', type=float, default=1024)
...
def main_worker(options, args):
    ...
    if options['loss'] in ['sphor']:
        use_mixup = True  
        print ("Mixup not used",str(options['loss']))
    else:
        use_mixup = False  
        print ("Mixup not used")
    ...
    # Create empty lists for the three groups
    non_fc_params = []
    fc_params = []
    ...
    params_list = [
        {'params': non_fc_params, 'lr': args.lr},
    ...
    for epoch in range(options['max_epoch']):
        ...
        if use_mixup:
            trainmixup(net, criterion, optimizer, trainloader, epoch=epoch, **options)
        else:
            train(net, criterion, optimizer, trainloader, epoch=epoch, **options)

        if options['eval_freq'] > 0 and (epoch+1) % options['eval_freq'] == 1 or (epoch+1) == options['max_epoch']:
