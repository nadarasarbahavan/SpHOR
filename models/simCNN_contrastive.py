...
class simCNN_contrastive(nn.Module):
    def __init__(self, classid_list, num_ABN=5, head='mlp', feature_dim=128, logit_dim=128):
        ...
        if head == 'linear':
            self.head = nn.Linear(self.feature_dim, self.logit_dim)
        elif head == 'mlp':
            self.head = nn.Sequential(
                nn.Linear(self.feature_dim, self.feature_dim),
                nn.ReLU(inplace=True),
                nn.Linear(self.feature_dim, self.logit_dim)
            )

        self.apply(weights_init_old)
        self.to(self.device)
...
class LinearClassifier(nn.Module):
    ...
    def forward_logitnorm(self, features):
        logits = self.fc(F.normalize(features, p=2, dim=1))
        return logits
    ...
    def predict_closed_v1(self, features):
...
class resnet_encoder(nn.Module):
    def __init__(self, classid_list, num_ABN=5, head='mlp', feature_dim=128, logit_dim=128):
        super(simCNN_contrastive, self).__init__()
        self.num_classes = len(classid_list)
        self.feature_dim = feature_dim
        self.logit_dim = logit_dim
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        self.feature_dim = resnet.fc.in_features  # ResNet-50 feature size is 2048

        # Projection head
        if head == 'linear':
            self.head = nn.utils.spectral_norm(nn.Linear(self.feature_dim, self.logit_dim))
        elif head == 'nospeclinear':
            self.head = nn.Linear(self.feature_dim, self.logit_dim)
        elif head == 'mlp':
            self.head = nn.Sequential(
                nn.Linear(self.feature_dim, self.feature_dim),
