...
class TimmResNetWrapper(torch.nn.Module):
    ...
    def forward(self, x, return_features=True, dummy_label=None):
        ...
        preds = self.resnet.fc(normalized_embedding)

        if return_features:
            return normalized_embedding, preds
        else:
            return preds
...
class TimmResNetWrapperDetached(torch.nn.Module):
...
def get_model(args, wrapper_class=None, evaluate=False, *args_, **kwargs):
    ...
    if args.model == 'timm_resnet50':
        ...
        # ResNet-50 trained from scratch (no ImageNet pretrain) - used for iNat21 OSR (Coarse-to-Fine paper)
        ...
        if args.cs:
            ...
            #model = timm.create_model('resnet50.ssl_in1k', num_classes=len(args.train_classes),pretrained=True)
