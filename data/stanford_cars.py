...
class CarsDataset(Dataset):
    ...
    def __init__(self, train=True, limit=0, data_dir=car_root, transform=None, metas=meta_default_path):
        ...
        data_dir = data_dir.format('train','train') if train else data_dir.format('test','test')
        #metas = metas.format('train_annos') if train else metas.format('test_annos_withlabels')
        metas = metas.format('train') if train else metas.format('test')
        ...
        df = pd.read_csv(metas, sep=',', header=None,
        ...
        for idx, row in df.iterrows():
            ...
            self.target.append(int(row['class_id']))
