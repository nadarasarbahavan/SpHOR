...
from data.inat2021osr import create_inat_dataset_funcs

from data.open_set_splits.osr_splits import osr_splits
from data.augmentations import get_transform
from config import osr_split_dir, inat21_osr_splits
...
# Register all inat21-osr-{aves,insecta}-{id,genus,family,order}-{1..7}hop datasets
get_dataset_funcs = create_inat_dataset_funcs(get_dataset_funcs)
...
def get_class_splits(dataset, split_idx=0, cifar_plus_n=10):
    ...
    if dataset in ('cifar-10-10', 'mnist', 'svhn'):
        ...
        # Load x-hop open-set split with species ids.
        # Pooling to super-class is handled by the dataloader.
        # Replace the tax_level with "id" to load the species-level split file.
