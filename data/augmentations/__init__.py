...
def get_transform(transform_type='default', image_size=32, args=None):
    ...
    if transform_type == 'default':
    ...
    elif transform_type == 'covariateshift':
        ...
        test_transform = transforms.Compose([
            transforms.Resize((image_size, image_size)),
            RandAugment(2, 30, args=args),  # very strong, random operations
            transforms.ColorJitter(brightness=0.8, contrast=0.8, saturation=0.8, hue=0.4),  # aggressive
            transforms.RandomSolarize(threshold=192.0, p=0.5),
            transforms.RandomPosterize(bits=3, p=0.5),
            transforms.RandomAffine(degrees=45, translate=(0.3, 0.3), scale=(0.5, 1.5)),  # large spatial changes
