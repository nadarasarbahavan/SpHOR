...
def get_default_hyperparameters(args):
    ...
    # if df.empty:
    #     raise ValueError(f"No rows found for Loss = '{args.loss}'")

    # # print("Filtered by Loss:\n", df)

    # filtered = df.loc[df['Dataset'] == args.dataset]

    # if filtered.empty:
    #     raise ValueError(f"No rows found for Dataset = '{args.dataset}' after filtering by Loss = '{args.loss}'")

    # # print("Filtered by Dataset:\n", filtered)

    # hyperparams = filtered.values[0][2:]
